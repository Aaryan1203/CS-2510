...
class Invader extends AGamePiece {
  ...
  public IGamePiece move(int x, int y) {
    return new Invader(this.loc.moveLocation(x, y), this.color, 
        this.bullets, this.size);
  }
...
class Spaceship extends AGamePiece {
  ...
  public IGamePiece move(int x, int y) {
    return new Spaceship(this.loc.moveLocation(x, y), this.color, 
        this.speed);
  }
...
class ExamplesGamePieces {
  ...
  IGamePiece ship3 = new Spaceship(Color.GREEN, 50);
  Spaceship ship2 = new Spaceship(this.loc1, Color.BLUE, 55);
  
  //tests for move
  boolean testMove(Tester t) {
    return t.checkExpect(this.ship1.move(3, 2), new Spaceship(new 
        Location(33, 92), Color.BLUE, 55)) &&
        t.checkExpect(this.invader1.move(3, 2), new Invader(new 
            Location(263, 182), Color.PINK, 30, 3));
  }
  
  //test for drawing a spaceship
  boolean testDraw(Tester t) {
    return t.checkExpect(this.ship1.draw(), new CircleImage(50, "solid", 
        Color.BLUE));
  }
  ...
  boolean testDrawing(Tester t) {
    ...
    WorldScene s1 = new WorldScene(300, 500).placeImageXY(img, this.loc1.x,
        this.loc1.y);
    WorldScene s2 = s1.placeImageXY(this.invader1.draw(), this.loc2.x, 
        this.loc2.y);
    c.show();
    c.drawScene(s2);
    return true;
